...
team_list AS (
    SELECT id, ROW_NUMBER() OVER (ORDER BY id) as rn FROM sample_teams
)
...
SELECT 
    ...
    END,
    true,
    ('1970-01-01'::date + (row_number() OVER () * INTERVAL '200 days'))::date,
    ...
    END,
    'HCP' || lpad((row_number() OVER ())::text, 6, '0'),
    ...
    'Consultant Psychiatrist',
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 8) + 1)
...
SELECT 
    ...
    END,
    true,
    ('1975-01-01'::date + (row_number() OVER () * INTERVAL '150 days'))::date,
    ...
    END,
    'NURSE' || lpad((row_number() OVER ())::text, 5, '0'),
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'female'
        WHEN 1 THEN 'male'
        ELSE 'female'
...
SELECT 
    ...
    END,
    true,
    ('1978-01-01'::date + (row_number() OVER () * INTERVAL '180 days'))::date,
    CASE row_number() OVER () % 4
    ...
    END,
    'PSYC' || lpad((row_number() OVER ())::text, 5, '0'),
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'female'
        WHEN 1 THEN 'male'
        ELSE 'female'
    ...
    'Clinical Psychologist',
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 6) + 1)
...
SELECT 
    ...
    END,
    true,
    ('1980-01-01'::date + (row_number() OVER () * INTERVAL '120 days'))::date,
    CASE row_number() OVER () % 4
    ...
    END,
    'SW' || lpad((row_number() OVER ())::text, 6, '0'),
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'female'
        WHEN 1 THEN 'male'
        ELSE 'female'
    ...
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 5) + 1)
FROM generate_series(1, 12) AS gs
...
SELECT 
    ...
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 4) + 1)
...
SELECT 
    ...
    END,
    true,
    ('1985-01-01'::date + (row_number() OVER () * INTERVAL '80 days'))::date,
    CASE row_number() OVER () % 5
    ...
    END,
    'SUPP' || lpad((row_number() OVER ())::text, 5, '0'),
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'female'
        WHEN 1 THEN 'male'
        ELSE 'female'
    ...
    END,
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 7) + 1)
...
SELECT 
    ...
    END,
    true,
    ('1976-01-01'::date + (row_number() OVER () * INTERVAL '250 days'))::date,
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'helen.clark@nhs.uk'
        WHEN 1 THEN 'mark.taylor@nhs.uk'
        ELSE 'susan.miller@nhs.uk'
    END,
    'PHARM' || lpad((row_number() OVER ())::text, 4, '0'),
    CASE row_number() OVER () % 3
        WHEN 0 THEN 'female'
        WHEN 1 THEN 'male'
        ELSE 'female'
    ...
    (SELECT id FROM team_list WHERE rn = ((row_number() OVER () - 1) % 3) + 1)
