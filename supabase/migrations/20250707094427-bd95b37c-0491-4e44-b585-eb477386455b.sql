-- Add 'discharged' to triage_status enum if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_enum e ON t.oid = e.enumtypid WHERE t.typname = 'triage_status' AND e.enumlabel = 'discharged') THEN
    ALTER TYPE triage_status ADD VALUE 'discharged';
  END IF;
END $$;

-- Update the discharge function to also set triage status to 'discharged'
...
BEGIN
  ...
  -- Always update referral status to discharged, set triage status to discharged, and clear waiting list fields
  UPDATE public.referrals 
  SET 
    status = 'discharged',
    triage_status = 'discharged',  -- NEW: Set triage status to discharged
