...
SELECT 
  ...
  -- Set appointment type based on referral characteristics
  CASE 
    WHEN r.priority = 'urgent' THEN 'consultation'
    WHEN r.specialty ILIKE '%surgery%' OR r.specialty ILIKE '%procedure%' THEN 'pre-admission'
    WHEN r.status = 'accepted' THEN 'follow-up'
    WHEN r.specialty ILIKE '%virtual%' OR r.specialty ILIKE '%telehealth%' THEN 'virtual'
    ELSE 'consultation'
