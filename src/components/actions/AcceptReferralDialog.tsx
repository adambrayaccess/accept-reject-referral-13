...
const triageStatusOptions: {
  ...
  value: 'pre-admission-assessment',
...
const AcceptReferralDialog = ({
    ...
    try {
      ...
      if (statusUpdated) {
        ...
        if (selectedStatus === 'refer-to-another-specialty') {
          ...
          const assignedTo = finalHCPId && finalHCPId !== 'unassigned' ? databaseHCPs.find(hp => hp.id === finalHCPId)?.name : 'team';
          successMessage = `The referral has been accepted and allocated to ${assignedTo} with status: ${triageStatusOptions.find(s => s.value === selectedStatus)?.label}.`;
        } else {
  ...
  return <Sheet open={isOpen} onOpenChange={setIsOpen}>
      {!referral.triageStatus && <SheetTrigger asChild>
          ...
          <div className="pr-4 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={selectedStatus} onValueChange={(value: TriageStatus) => setSelectedStatus(value)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select status" />
                  </SelectTrigger>
                  <SelectContent>
                    {triageStatusOptions.map(status => <SelectItem key={status.value} value={status.value}>
                        {status.label}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              {selectedStatus === 'refer-to-another-specialty' ? <div className="space-y-2">
                  ...
                  <Select value={selectedSpecialty} onValueChange={setSelectedSpecialty}>
                    ...
                    <SelectContent>
                      {specialties.map(specialty => <SelectItem key={specialty.id} value={specialty.id}>
                          {specialty.name}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                </div> : <>
                  {specialtyId && <TeamSelector specialtyId={specialtyId} selectedTeamId={selectedTeamId} selectedHCPId={selectedHCPId} onTeamChange={handleTeamChange} onHCPChange={handleHCPChange} />}
                  
                  {(!specialtyId || !selectedTeamId) && <div className="space-y-2">
                      ...
                      <Select value={selectedProfessional} onValueChange={setSelectedProfessional}>
                        ...
                        <SelectContent>
                          {isLoadingHCPs ? <SelectItem value="loading" disabled>
                              Loading healthcare professionals...
                            </SelectItem> : databaseHCPs.filter(hcp => hcp.active !== false).map(professional => <SelectItem key={professional.id} value={professional.id}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6">
              <Button onClick={handleAccept} disabled={isSubmitting || !isFormValid()} className="flex-1 bg-[#006262]">
                {isSubmitting ? "Processing..." : "Accept Referral"}
              </Button>
              <Button variant="outline" onClick={() => setIsOpen(false)} disabled={isSubmitting} className="flex-1">
                Cancel
              </Button>
            </div>
          </div>
