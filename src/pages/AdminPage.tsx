...
const AdminPage = () => {
  ...
  try {
    console.log('AdminPage: Calling useDashboardData with specialties:', currentSpecialty ? [currentSpecialty] : []);
    const { referrals, isLoading, handleRefresh } = useDashboardData(currentSpecialty ? [currentSpecialty] : []);
    console.log('AdminPage: useDashboardData result:', { referralsCount: referrals?.length, isLoading });
    
    console.log('AdminPage: Calling useAdminStatistics');
    const { specialtyStats, overallStats } = useAdminStatistics(referrals);
    console.log('AdminPage: useAdminStatistics result:', { specialtyStatsCount: specialtyStats?.length, overallStats });
    ...
    console.error('AdminPage: Error occurred:', error);
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-2">Error Loading Admin Page</h1>
          <p className="text-gray-600 mb-4">Check the console for more details</p>
